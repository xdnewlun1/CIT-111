...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    total_mass = 0
    for group in symbol_quantity_list:
        total_mass += periodic_table[group[0]][1] * group[1]
    return total_mass


def main():
    formula = input("Molecular Formula: ")
    mass = float(input("Mass in Grams of the sample: "))
    periodic_table = make_periodic_table()
    formula = parse_formula(formula, periodic_table)
